<?php
	...
	$check_recurso = $check_recurso->query("SELECT * FROM recursos WHERE recurso_id='$resource_id_del'");

	if ($check_recurso->rowCount() == 1) {
		$datos = $check_recurso->fetch();

		$eliminar_recurso = conexion();
		$eliminar_recurso = $eliminar_recurso->prepare("DELETE FROM recursos WHERE recurso_id=:id");
